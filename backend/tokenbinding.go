...
func decryptTokenEncryptedResponse(parsed map[string]string, session *tokenBindingSession) error {
	...
	tokenKey, encryptedToken := encryptedTokenField(parsed)
	if encryptedToken == "" {
		return errors.New("auth response returned TokenEncrypted=1 but no encrypted token field was present")
	}
	...
	parsed[tokenKey] = plaintext
	parsed["Auth"] = plaintext
	return nil
}

func encryptedTokenField(parsed map[string]string) (string, string) {
	for _, key := range []string{"it", "Auth", "SID", "LSID"} {
		if parsed[key] != "" {
